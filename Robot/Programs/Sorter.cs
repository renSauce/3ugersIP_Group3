def sorter():
  ...
  set_tcp(p[0.0,-0.09,0.2165,0.524,0.0,0.0])
  ...
  set_target_payload(1.122000, [0.000000, -0.003800, 0.086500], [0.001865, 0.001865, 0.001865, 0.000000, 0.000000, 0.000000])
  # begin: URCap Installation Node
  #   Source: UR Connect, 2.3.3, Universal Robots
  #   Type: UR Connect
  # end: URCap Installation Node
  # begin: URCap Installation Node
  #   Source: OnRobot, 6.4.1, OnRobot A/S
  #   Type: OnRobot Setup
  sleep(1.0)
  if (False):
  ...
  ON_DI_SECONDARY = 2
  ON_DI_DUAL = 3
  on_conn_ip = "172.20.254.106"
  on_tool_ip = "172.20.254.106"
  on_device_socket_port = 51234
  on_conn_xmlrpc = rpc_factory("xmlrpc", "http://172.20.254.106:41414")
  on_tool_xmlrpc = rpc_factory("xmlrpc", "http://172.20.254.106:41414")
  ON_DEBUG_LOG = False
  on_isMetric = True
  ...
  rg_mounting_angle_arr[0] = 0.523599
  rg_fingertip_arr[0] = 4.599999904632568
  rg_Depth_arr[0] = 38.29999923706054
  ...
  sync()
  run rg_dataProcess_thread()
  sync()
  run rg_monitor_thread()
  sync()
  
  #======    End of OnRobot RG Run    ====== START#
  # end: URCap Installation Node
  step_count_b93b391d_2a73_430a_b5b3_63453aa8ff51 = 0.0
  thread Step_Counter_Thread_493bfbe9_cb8a_46d4_ba34_385afc693ffa():
    while (True):
      step_count_b93b391d_2a73_430a_b5b3_63453aa8ff51 = step_count_b93b391d_2a73_430a_b5b3_63453aa8ff51 + 1.0
      sync()
    end
  end
  run Step_Counter_Thread_493bfbe9_cb8a_46d4_ba34_385afc693ffa()
  ...
  global Waypoint_1_p=p[.280110559084, -.611913470044, .359921050877, 2.069099464367, .075686481774, .041202226773]
  global Waypoint_1_q=[1.939499616622924, -0.8269779247096576, 0.7057250181781225, -3.0464784107603977, -1.8695719877826136, 3.1333689689636226]
  global Waypoint_2_p=p[.347036203791, -.406705469415, -.117174798730, -3.112408844798, .004248072513, -.053327000923]
  global Waypoint_3_p=p[-.206331968965, -.497838673522, -.064026522761, -2.613660637500, 1.335120535459, -.025721254140]
  global OrderDropPose=p[-.206331968965, -.497838673522, -.064026522761, -2.613660637500, 1.335120535459, -.025721254140]
  global ResortDropPose=p[-.206331968965, -.197838673522, -.064026522761, -2.613660637500, 1.335120535459, -.025721254140]
  global SortBlueToOrder=True
  global SortRedToOrder=True
  global SortGreenToOrder=True
  global SortYellowToOrder=True
  global BlueRemaining=0
  global RedRemaining=0
  global GreenRemaining=0
...
if (eyes_cnt > 0):
  ...
  # existing pick logic continues
else:
  set_standard_digital_out(7, False)  # stop conveyor
  halt
  end
      # begin: URCap Program Node 
      #   Source: OnRobot, 6.4.1, OnRobot A/S
      #   Type: Eyes Get Workpiece BLÃ…
      $ 7 "Eyes Get Workpiece" "noBreak"
      eyes_workp_type = -1
      eyes_workp_cnt = -1
      ...
      if (eyes_workp_type != -1):
        $ 8 "EyesWorkpCountâ‰”eyes_workp_cnt" "noBreak"
        global EyesWorkpCount=eyes_workp_cnt
        $ 9 "EyesWorkpPoseâ‰”eyes_workp_pose" "noBreak"
        global EyesWorkpPose=eyes_workp_pose
        $ 10 "EyesWorkpieceâ‰”eyes_workp_type" "noBreak"
        global EyesWorkpiece=eyes_workp_type
        $ 11 "Commands" "noBreak"
        # begin: URCap Program Node
        #   Source: OnRobot, 6.4.1, OnRobot A/S
        #   Type: Eyes Pick
        $ 12 "Eyes Pick" "noBreak"
        if (pose_dist(EyesWorkpPose, p[0,0,0,0,0,0]) > 0.005):
          global EyesPickPose=EyesWorkpPose
          global EyesPickAppr=pose_trans(EyesWorkpPose, p[0,0, -0.0,0,0,0])
          $ 13 "MoveJ" "noBreak"
          $ 14 "EyesPickAppr" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          movej(EyesPickAppr, a=1.3962634015954636, v=1.0471975511965976)
          $ 15 "MoveL" "noBreak"
          $ 16 "EyesPickPose" "noBreak"
          ...
          while (True):
            ...
            if (move_thread_flag_16 > 1):
              ...
              $ 17 "Until (reached_waypoint)" "noBreak"
              $ 18 "Waypoint_2" "noBreak"
              set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          ...
          #   Type: RG Grip
          $ 19 "RG Grip" "noBreak"
on_return = rg_grip(0.0, 12.0, tool_index = 0, blocking = True, depth_comp = False, popupmsg = True)
rg_payload_set(mass = 0.0, tool_index = 0, use_guard = True)
          # end: URCap Program Node
          $ 20 "MoveL" "noBreak"
          $ 21 "Waypoint_3" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
...
rg_payload_set(mass = 0.0, tool_index = 0, use_guard = True)
      ...
      # begin: URCap Program Node
      #   Source: OnRobot, 6.4.1, OnRobot A/S
      #   Type: Eyes Get Workpiece RÃ˜D
      $ 23 "Eyes Get Workpiece" "noBreak"
      eyes_workp_type = -1
      eyes_workp_cnt = -1
      eyes_workp_pose = p[0,0,0,0,0,0]
      eyes_workp_pose = eyes_getworkpiecepose(2)
      eyes_workp_type = eyes_getworkpiecetype()
      eyes_workp_matc = eyes_getworkpiecematch()
      eyes_workp_eval = eyes_getworkpieceeval()
      eyes_workp_cnt = eyes_getworkpiececount()
      if (eyes_workp_type != -1):
        $ 24 "EyesWorkpCountâ‰”eyes_workp_cnt" "noBreak"
        global EyesWorkpCount=eyes_workp_cnt
        $ 25 "EyesWorkpPoseâ‰”eyes_workp_pose" "noBreak"
        global EyesWorkpPose=eyes_workp_pose
        $ 26 "EyesWorkpieceâ‰”eyes_workp_type" "noBreak"
        global EyesWorkpiece=eyes_workp_type
        $ 27 "Commands" "noBreak"
        # begin: URCap Program Node
        #   Source: OnRobot, 6.4.1, OnRobot A/S
        #   Type: Eyes Pick
        $ 28 "Eyes Pick" "noBreak"
        if (pose_dist(EyesWorkpPose, p[0,0,0,0,0,0]) > 0.005):
          global EyesPickPose=EyesWorkpPose
          global EyesPickAppr=pose_trans(EyesWorkpPose, p[0,0, -0.0,0,0,0])
          $ 29 "MoveJ" "noBreak"
          $ 30 "EyesPickAppr" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          movej(EyesPickAppr, a=1.3962634015954636, v=1.0471975511965976)
          $ 31 "MoveL" "noBreak"
          $ 32 "EyesPickPose" "noBreak"
          ...
          while (True):
            ...
            if (move_thread_flag_32 > 1):
              ...
              $ 33 "Until (reached_waypoint)" "noBreak"
              $ 34 "Waypoint_2" "noBreak"
              set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
              movel(Waypoint_2_p, a=1.2, v=0.25)
              break
            end
...
rg_payload_set(mass = 0.0, tool_index = 0, use_guard = True)
      ...
      # begin: URCap Program Node
      #   Source: OnRobot, 6.4.1, OnRobot A/S
      #   Type: Eyes Get Workpiece GRÃ˜N
      $ 39 "Eyes Get Workpiece" "noBreak"
      eyes_workp_type = -1
      eyes_workp_cnt = -1
      eyes_workp_pose = p[0,0,0,0,0,0]
      eyes_workp_pose = eyes_getworkpiecepose(3)
      eyes_workp_type = eyes_getworkpiecetype()
      eyes_workp_matc = eyes_getworkpiecematch()
      eyes_workp_eval = eyes_getworkpieceeval()
      eyes_workp_cnt = eyes_getworkpiececount()
      if (eyes_workp_type != -1):
        $ 40 "EyesWorkpCountâ‰”eyes_workp_cnt" "noBreak"
        global EyesWorkpCount=eyes_workp_cnt
        $ 41 "EyesWorkpPoseâ‰”eyes_workp_pose" "noBreak"
        global EyesWorkpPose=eyes_workp_pose
        $ 42 "EyesWorkpieceâ‰”eyes_workp_type" "noBreak"
        global EyesWorkpiece=eyes_workp_type
        $ 43 "Commands" "noBreak"
        # begin: URCap Program Node
        #   Source: OnRobot, 6.4.1, OnRobot A/S
        #   Type: Eyes Pick
        $ 44 "Eyes Pick" "noBreak"
        if (pose_dist(EyesWorkpPose, p[0,0,0,0,0,0]) > 0.005):
          global EyesPickPose=EyesWorkpPose
          global EyesPickAppr=pose_trans(EyesWorkpPose, p[0,0, -0.0,0,0,0])
          $ 45 "MoveJ" "noBreak"
          $ 46 "EyesPickAppr" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          movej(EyesPickAppr, a=1.3962634015954636, v=1.0471975511965976)
          $ 47 "MoveL" "noBreak"
          $ 48 "EyesPickPose" "noBreak"
          ...
          while (True):
            ...
            if (move_thread_flag_48 > 1):
              ...
              $ 49 "Until (reached_waypoint)" "noBreak"
              $ 50 "Waypoint_2" "noBreak"
              set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
              movel(Waypoint_2_p, a=1.2, v=0.25)
              break
            end
...
rg_payload_set(mass = 0.0, tool_index = 0, use_guard = True)
          # end: URCap Program Node
          $ 52 "MoveL" "noBreak"
          $ 53 "Waypoint_3" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          drop_pose = ResortDropPose
          if (GreenRemaining > 0):
          drop_pose = OrderDropPose
          GreenRemaining = GreenRemaining - 1
          end
          movel(drop_pose, a=1.2, v=0.25)
          # begin: URCap Program Node
          #   Source: OnRobot, 6.4.1, OnRobot A/S
...
rg_payload_set(mass = 0.0, tool_index = 0, use_guard = True)
      ...
      # begin: URCap Program Node
      #   Source: OnRobot, 6.4.1, OnRobot A/S
      #   Type: Eyes Get Workpiece GUL
      $ 55 "Eyes Get Workpiece" "noBreak"
      eyes_workp_type = -1
      eyes_workp_cnt = -1
      eyes_workp_pose = p[0,0,0,0,0,0]
      eyes_workp_pose = eyes_getworkpiecepose(4)
      eyes_workp_type = eyes_getworkpiecetype()
      eyes_workp_matc = eyes_getworkpiecematch()
      eyes_workp_eval = eyes_getworkpieceeval()
      eyes_workp_cnt = eyes_getworkpiececount()
      if (eyes_workp_type != -1):
        $ 56 "EyesWorkpCountâ‰”eyes_workp_cnt" "noBreak"
        global EyesWorkpCount=eyes_workp_cnt
        $ 57 "EyesWorkpPoseâ‰”eyes_workp_pose" "noBreak"
        global EyesWorkpPose=eyes_workp_pose
        $ 58 "EyesWorkpieceâ‰”eyes_workp_type" "noBreak"
        global EyesWorkpiece=eyes_workp_type
        $ 59 "Commands" "noBreak"
        # begin: URCap Program Node
        #   Source: OnRobot, 6.4.1, OnRobot A/S
        #   Type: Eyes Pick
        $ 60 "Eyes Pick" "noBreak"
        if (pose_dist(EyesWorkpPose, p[0,0,0,0,0,0]) > 0.005):
          global EyesPickPose=EyesWorkpPose
          global EyesPickAppr=pose_trans(EyesWorkpPose, p[0,0, -0.0,0,0,0])
          $ 61 "MoveJ" "noBreak"
          $ 62 "EyesPickAppr" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          movej(EyesPickAppr, a=1.3962634015954636, v=1.0471975511965976)
          $ 63 "MoveL" "noBreak"
          $ 64 "EyesPickPose" "noBreak"
          ...
          while (True):
            ...
            if (move_thread_flag_64 > 1):
              ...
              $ 65 "Until (reached_waypoint)" "noBreak"
              $ 66 "Waypoint_2" "noBreak"
              set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
              movel(Waypoint_2_p, a=1.2, v=0.25)
              break
            end
...
rg_payload_set(mass = 0.0, tool_index = 0, use_guard = True)
          # end: URCap Program Node
          $ 68 "MoveL" "noBreak"
          $ 69 "Waypoint_3" "noBreak"
          set_tcp(p[0.0,-0.09,0.216485,0.523599,0.0,0.0])
          drop_pose = ResortDropPose
          if (YellowRemaining > 0):
          drop_pose = OrderDropPose
          YellowRemaining = YellowRemaining - 1
          end
          movel(drop_pose, a=1.2, v=0.25)
          # begin: URCap Program Node
          #   Source: OnRobot, 6.4.1, OnRobot A/S
